...
export function getCDNConfig(): CDNConfig {
  ...
  const provider = (process.env.NEXT_PUBLIC_CDN_PROVIDER as CDNProvider) || 'none';
  ...
  return {
    ...
    imageBaseUrl: process.env.NEXT_PUBLIC_CDN_IMAGE_URL || baseUrl,
    staticBaseUrl: process.env.NEXT_PUBLIC_CDN_STATIC_URL || baseUrl,
    imageTransformation: {
      enabled: process.env.NEXT_PUBLIC_CDN_IMAGE_TRANSFORM === 'true',
      quality: parseInt(process.env.NEXT_PUBLIC_CDN_IMAGE_QUALITY || '80', 10),
      format: (process.env.NEXT_PUBLIC_CDN_IMAGE_FORMAT as CDNConfig['imageTransformation']['format']) || 'auto',
      fit: (process.env.NEXT_PUBLIC_CDN_IMAGE_FIT as CDNConfig['imageTransformation']['fit']) || 'cover',
    },
    cache: {
      defaultMaxAge: parseInt(process.env.NEXT_PUBLIC_CDN_CACHE_DEFAULT || '86400', 10),
      imageMaxAge: parseInt(process.env.NEXT_PUBLIC_CDN_CACHE_IMAGES || '31536000', 10),
      staticMaxAge: parseInt(process.env.NEXT_PUBLIC_CDN_CACHE_STATIC || '31536000', 10),
    },
  };
}
