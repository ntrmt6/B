...
// Helper to handle API responses with proper error code handling
  ...
  if (!response.ok) {
    ...
    if (response.status === 401) {
      ...
      if (code === 'TOKEN_EXPIRED') {
        throw Object.assign(new Error('Session expired. Please log in again.'), { code });
      }
      if (code === 'ACCOUNT_DEACTIVATED') {
        throw Object.assign(new Error('Your account has been deactivated. Contact support.'), { code });
      }
      throw Object.assign(new Error(data.error || 'Authentication failed'), { code });
    }

    // Handle 402 Subscription expired
    if (response.status === 402) {
      throw Object.assign(new Error(data.message || data.error || 'Your subscription has expired. Please renew to continue.'), { code: data.code || 'SUBSCRIPTION_EXPIRED' });
    }

    // Handle 403 Forbidden
    if (response.status === 403) {
      throw Object.assign(new Error(data.error || 'You do not have permission to perform this action.'), { code: data.code || 'FORBIDDEN' });
    }

    // Handle 503 Service unavailable
    if (response.status === 503) {
      throw Object.assign(new Error(data.error || 'Service temporarily unavailable. Please try again.'), { code: data.code || 'SERVICE_UNAVAILABLE' });
    }

    // Handle validation errors (Zod errors)
    if (data.error && Array.isArray(data.error)) {
      const errorMessage = data.error.map((e: any) => e.message).join(', ');
      throw Object.assign(new Error(errorMessage), { code: 'VALIDATION_ERROR' });
    }
    
    // Handle string or object errors with backend error code
    const errorMessage = typeof data.error === 'string' ? data.error : data.message || `HTTP Error ${response.status}`;
    throw Object.assign(new Error(errorMessage), { code: data.code || 'API_ERROR' });
  }
