...
export const ProductCardSkeleton = () => (
  <div className="bg-white rounded-xl overflow-hidden border border-gray-100/80 shadow-[0_1px_3px_rgba(0,0,0,0.05)]" style={{ contain: 'layout' }}>
    <div className="bg-gray-50/50" style={{ aspectRatio: '1/1' }}><Bone className="w-full h-full rounded-none"/></div>
    <div className="px-1 pb-1 pt-0.5 flex flex-col gap-0.5">
      <div className="flex items-center gap-0.5"><Bone className="h-2.5 w-2.5 rounded-full"/><Bone className="h-2.5 w-6"/></div>
      <div className="min-h-[28px] sm:min-h-[32px] md:min-h-[40px]"><Bone className="h-3 md:h-3.5 w-full"/></div>
      <div className="flex items-baseline gap-1.5 mt-auto"><Bone className="h-4 w-14"/><Bone className="h-3 w-10"/></div>
      <div className="flex gap-0.5 mt-0.5"><Bone className="w-8 h-7 md:h-8 rounded-lg"/><Bone className="flex-1 h-7 md:h-8 rounded-lg"/></div>
    </div>
  </div>
);
...
export const HeroSkeleton = () => <div className="max-w-[1720px] mx-auto px-2 md:px-4 lg:px-0 pb-1"><div className="bg-gray-200 animate-pulse w-full rounded-lg sm:rounded-xl" style={{ aspectRatio: '20/6', maxHeight: '500px' }}/></div>;
export const ProductGridSkeleton = ({ count = 6 }: { count?: number }) => <div className="grid grid-cols-2 gap-2 sm:gap-3 md:gap-4 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5">{arr(count).map((_, i) => <ProductCardSkeleton key={i}/>)}</div>;
export const CategorySkeleton = ({ count = 8 }: { count?: number }) => <div className="flex gap-0.5 overflow-hidden py-0.5">{arr(count).map((_, i) => <div key={i} className="flex-shrink-0 flex items-center gap-1 px-1.5 py-0.5 sm:px-2 sm:py-1 rounded-full bg-white border border-gray-100 shadow-sm"><Bone className="w-7 h-7 sm:w-9 sm:h-9 rounded-full"/><Bone className="h-3 w-12 sm:w-16"/></div>)}</div>;
...
export const ProductDetailSkeleton = () => (
  <div className="min-h-screen font-sans text-slate-900 pb-28 md:pb-0" style={{ background: 'linear-gradient(to bottom, #f0f4f8, #e8ecf1)' }}>
    <StoreHeaderSkeleton />
    <div className="max-w-[1500px] mx-auto px-4 sm:px-6 lg:px-8 pt-2 pb-4">
      <div className="flex flex-col lg:flex-row gap-4">
        <div className="flex-1">
          <div className="bg-white/90 rounded-2xl p-4 sm:p-6">
            <div className="flex flex-col md:flex-row gap-4 md:gap-10">
              <div className="w-full md:w-1/2 space-y-3">
                <Bone className="aspect-square rounded-[24px]"/>
                <div className="flex gap-3">{arr(4).map((_, i) => <Bone key={i} className="w-20 h-20 rounded-xl flex-shrink-0"/>)}</div>
              </div>
              <div className="w-full md:w-1/2 space-y-4">
                <Bone className="h-7 md:h-9 w-3/4"/>
                <div className="flex items-center gap-3"><Bone className="h-4 w-20"/><Bone className="h-4 w-16"/></div>
                <div className="flex items-baseline gap-3"><Bone className="h-8 md:h-10 w-24"/><Bone className="h-5 w-16"/></div>
                <Bone className="h-10 w-36 rounded-full"/>
                <div className="space-y-3">
                  <Bone className="h-3 w-12"/><div className="flex gap-2">{arr(3).map((_, i) => <Bone key={i} className="h-9 w-16 rounded-lg"/>)}</div>
                </div>
                <div className="hidden md:flex gap-3"><Bone className="flex-1 h-12 rounded-xl"/><Bone className="flex-1 h-12 rounded-xl"/></div>
              </div>
            </div>
          </div>
        </div>
        <div className="w-full lg:w-80 space-y-4"><Bone className="h-48 rounded-[24px]"/><Bone className="h-32 rounded-[24px]"/></div>
      </div>
    </div>
  </div>
);
...
export const StoreHeroSkeleton = () => (
  <div className="max-w-[1720px] mx-auto px-2 md:px-4 lg:px-0 pb-1">
    <div className="bg-gray-200 animate-pulse w-full rounded-lg sm:rounded-xl" style={{ aspectRatio: '20/6', maxHeight: '500px' }}/>
  </div>
);
...
export const StoreProductSectionSkeleton = ({ title, count = 5 }: { title?: string; count?: number }) => (
  <div className="py-2">
    {title && <Bone className="h-5 w-36 mb-2 rounded"/>}
...
export const StorePageSkeleton = () => (
  <div className="min-h-screen font-sans text-slate-900" style={{ background: '#ffffff' }}>
    ...
    <div className="max-w-[1720px] mx-auto px-0.5 sm:px-1 lg:px-1.5 space-y-0.5 sm:space-y-1 pb-4" style={{ minHeight: '680px' }}>
