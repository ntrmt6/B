...
/** Configuration for a single dashboard section's access level */
export interface SectionAccessConfig {
  /** Whether the section is accessible */
  enabled: boolean;
  /** Whether the section requires a premium subscription upgrade */
  premium: boolean;
}

/** Per-tenant feature configuration stored in the backend */
export interface TenantFeaturesConfig {
  sections: Record<string, SectionAccessConfig>;
  updatedBy?: string;
  updatedAt?: string;
}

/** Definition of a dashboard section for the feature control UI */
